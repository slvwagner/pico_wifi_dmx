  ...
  await page.route('http://pico.test/**', async route => {
    ...
    if (url === 'http://pico.test/status.json') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ ok: true, firmware_version: appVersion, dmx: { channels: 512, frame_count: 42 } })
      });
      return;
    }
    await route.fulfill({ status: 200, contentType: 'application/json', body: '{"ok":true}' });
...
test.describe('Show Run page', () => {
  ...
  test('shows primary show output health instead of a Pico URL editor', async ({ page }) => {
    ...
    await expect(page.locator('header [data-pico-fleet-status]')).toHaveText(`1/1 Pico online · firmware ${appVersion}`);
