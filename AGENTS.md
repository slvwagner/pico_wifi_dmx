...
- `http://192.168.0.12/dmx/` and `E:\Software\xampp\htdocs\dmx` are the user's working environment and contain user-owned show data.
- `http://192.168.0.12/dmx-test/` and `E:\Software\xampp\htdocs\dmx-test` are the automation playground.
- Never infer safety from the hostname. `localhost/dmx/` is the same protected environment as `192.168.0.12/dmx/` when served by this XAMPP installation.
...
4. Synchronize the automation playground with `scripts/update_xampp_server.ps1 -AppFolder dmx-test -BaseUrl http://192.168.0.12/dmx-test/`.
5. Deploy user-facing source changes only with `scripts/update_xampp_server.ps1 -AppFolder dmx -BaseUrl http://192.168.0.12/dmx/`.
