...
- **Fixture Library** — load fixture profiles from the converted Open Fixture Library catalog, merge edited show profiles with reusable library modes in either direction, and export/import the complete catalog as a compressed ZIP.
- **Show Run operator page** — arrange Groups, Fixtures, Scenes, Palettes, Room Planes, Live Controls, masters, MIDI, and Pico playback in a configurable card matrix. Card and tile layouts are saved to the server, cards can be repeated for separate operator banks, and the sticky header provides a state-aware full-screen control on supported PC and iPad browsers.
...
- **Chaser** — create step-based chases, define participating controls, add/capture/duplicate/reorder steps, preview recalled chases or individual steps directly on DMX, use browser playback with direction and ping-pong modes, and upload chases into Pico slots for standalone playback.
...
To regenerate both compact enrichment sidecars without writing the curated fixture library, run:

```powershell
.\scripts\build_fixture_library.ps1 -SidecarsOnly -MetadataOutputPath web/assets/fixture-metadata.json -CapabilitiesOutputPath web/assets/fixture-capabilities.json
```

The capability sidecar keeps normalized OFL channel functions and ranges. Segmented channels can therefore appear as named option controls for shutter/strobe, programs, effects, prisms, and maintenance functions, while continuous functions retain range and speed metadata for bounded controls where applicable.
