...
- **Show Run operator page** — arrange Groups, Fixtures, Scenes, Palettes, Pixel Matrices, Room Planes, Live Controls, masters, MIDI, and Pico playback in a configurable card matrix. Operator-critical cards can be moved into a fixed, independently scrolling single-column sidebar with 1–12 configurable rows. The sidebar uses the same visual design and shared adjustable width as the Controller toolbox rail. Card, sidebar, and tile layouts are saved to the server, cards can be repeated for separate operator banks, and the adaptive sticky header keeps navigation visible while reporting show-wide Pico output health.
...
An asset suffix such as `?v=1.3.1-1` is a browser-cache revision within application version `1.3.1`; `-1` is not an additional release number. Incrementing it forces browsers and iPad Home Screen installations to load changed shared CSS or JavaScript.
...
Complete setup exports also include a `project` block, `setupFormatVersion`, and `minimumAppVersion`. Before confirmation or any server write, Import runs the setup through its format migration guard and compares the minimum application version with the running release using semantic-version precedence. Older supported setup formats are upgraded step-by-step to the current format. Files with a newer setup format, a newer minimum application version, or malformed minimum-version metadata are refused with a clear update-software or validation message.
...
Multiple controller or operator browsers may use the same installation. Chaser-slot, Effects-slot, and shared UI-state updates use locked read-modify-write transactions so nearly simultaneous requests cannot replace a newer setup with an older full-file snapshot. Pico HTTP request bodies and generated responses are also owned per connection, allowing status polling, control writes, and slot uploads to overlap without sharing in-flight buffers.
...
The Windows customer package configures its private PHP runtime with 512 MB of memory, a 120-second processing limit, and 128 MB request/upload limits. PHP expands the roughly 55 MB bundled catalog substantially while decoding and merging it; smaller runtime limits can terminate the endpoint before it returns JSON and appear in the Controller as `Invalid server response`.

OFL color data is retained as display-ready hex previews. For split-color wheel positions, the converter combines OFL `WheelSlot` capabilities, their DMX ranges, and either their direct `colors` list or the colors referenced by `slotNumberStart` and `slotNumberEnd`. The generated option remains a `WheelSlot`, keeps every color, and exposes its complete range through a **Split position** slider; this represents the mechanical wheel position between the two colors rather than a calibrated optical mixing percentage. Single-color slots remain fixed, while genuine `WheelRotation` ranges continue to expose **Rotation speed**. The first split color also remains available through the legacy `color` field. LEE and Rosco numbers found in OFL capability text are stored as structured filter references; their hex values remain OFL/manufacturer approximations for the fixture rather than calibrated spectral conversions. RGB fixtures with additional UV, lime, indigo, cyan, magenta, yellow, warm-white, or cold-white channels expose every emitter in one advanced color control, while the normal RGB color picker continues to set the red, green, and blue emitters and leaves the additional emitter sliders independently adjustable.
