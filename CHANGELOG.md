...
- Added an Ubuntu customer installer workflow that builds a versioned, checksummed `.deb` package with a systemd PHP service, Chromium app window with a dedicated profile and dark frame, Applications-menu launcher, localhost-only default, explicit trusted-LAN/UFW configuration, pre-upgrade show snapshots, and persistent data preserved on removal.
