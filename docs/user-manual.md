...
Use **Guided wheel editor** when creating or correcting indexed controls such as color wheels, gobo wheels, and shutter/strobe ranges. The modal lets you edit each option as fields: option name, DMX range start/end, function type, slot number, speed labels, and optional visual color/image data. This is the recommended way to define rich functions such as `WheelShake`, `WheelRotation`, and `ShutterStrobe` without memorizing the raw text syntax.
