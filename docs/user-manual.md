...
| Show Run | Operator page for recalling saved groups, scenes, palettes, and Pico chaser/motion slots without editing setup |
...
The **Show Run** page is the operator page. Use it when the show has already been prepared on the Fixture Controller, Chaser, and Motion FX pages and you want fewer editing controls on screen.
...
- Mirrored Pico motion slots from `motion_setup.php?slots`
...
**Pico Chaser Playback** and **Pico Motion Playback** show the slots that were uploaded and mirrored from the Chaser and Motion FX pages. They also read live Pico slot state, so a slot that is loaded on the Pico can still appear even when the XAMPP mirror is empty.
...
Use the **Pico Motion Playback** controls the same way: choose a slot, set **BPM**, then **Start Slot**, **Set BPM**, or **Stop Slot**. Starting a mirrored slot reloads its payload before running it; starting a live-only Pico slot starts the already-loaded Pico slot without overwriting it.

The global **Stop Chaser**, **Stop Motion**, and **Stop All Playback** buttons call the matching Pico stop endpoints.
