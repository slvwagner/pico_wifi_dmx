...
With the customer installer, open **Pico DMX Controller** from the desktop or Start Menu. The local address is:
...
From an iPad, phone, or another computer, use the controller computer's LAN address or hostname. A customer installation that enabled **Allow access from iPads and PCs on the private network** uses an address such as `http://192.168.0.50:8090/`; a XAMPP development installation may use `http://192.168.0.50/dmx/`. This computer URL is only the address of the web interface and server-side show storage.
...
- **Show Run**, **Chaser**, **Effects**, and **Room Plane** currently use the show's first output for direct Pico operations. Their shared header still reports fleet availability, but their live/playback request paths are not yet fully multi-output.
...
1. Run `pico-dmx-controller-<VERSION>-windows-x64.exe` as an administrator.
2. Keep the default installation folder unless company policy requires another location.
3. Enable **Allow access from iPads and PCs on the private network** only when another trusted device needs the controller. This creates a Windows Private-profile firewall rule for TCP port 8090. It does not expose the app through the router or public internet.
4. Finish the installer and select **Open Pico DMX Controller**, or use its desktop/Start Menu shortcut later. The shortcut opens a dedicated application window rather than a normal browser tab.
5. For an iPad on the same private Wi-Fi, open `http://<controller-pc-address>:8090/`. The PC and the Pico controllers must be reachable from that Wi-Fi.
6. Configure the hardware in **DMX Outputs**, then use **Export Show** to keep a portable backup in addition to the server copy.

The application window has normal minimize, maximize, and close controls. Press **F11** to enter fullscreen and **Escape** to leave it. Fullscreen also keeps an **Exit full screen** button and **Close application** button visible. The tray icon offers **Open**, **Toggle full screen**, and **Exit application**. Opening the shortcut again activates the existing window instead of creating another instance.

Closing the application window exits only the operator GUI. The web server remains available to iPads and other devices as the automatic `PicoDmxController` Windows service, so the operator does not need to open a XAMPP control panel. Application binaries live below `C:\Program Files\Pico DMX Controller`; show data lives separately below `C:\ProgramData\Pico DMX Controller\data`. An upgrade snapshots existing data below the neighboring `backups` folder before replacing application files. Uninstall removes the app and service but preserves the `ProgramData` show data deliberately.

Do not forward port 8090 from the internet router. The LAN option is intended for a trusted private lighting network. An unsigned development installer can display a Windows SmartScreen warning; customer release installers should carry the supplier's Authenticode signature.
...
1. Open the controller URL: normally `http://localhost:8090/` for a Windows customer installation, or the configured XAMPP URL in a development environment.
...
Show Run, Chaser, Effects, and Room Plane still use the first output for direct Pico operations that currently target one controller, such as autonomous Chaser/Effects slots. GPIO Control, DMX Buffer Monitor, and Pico Performance Test provide their own output selectors. Fixture DMX routing and Pixel Matrix output remain universe-aware and follow each fixture's assigned output.
