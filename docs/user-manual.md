...
3. On **Controller port**, keep the default `8090` or enter another unused port from `1024` to `65535`. The installer detects a conflict and asks for a different port. An upgrade starts with the currently installed port so existing bookmarks continue to work unless you deliberately change it.
