...
Use **Guided wheel editor** when creating or correcting indexed controls such as color wheels and gobo wheels. The modal lets you edit each option as fields: option name, DMX range start/end, function type, slot number, speed labels, and optional visual color/image data. This is the recommended way to define rich functions such as `WheelShake` and `WheelRotation` without memorizing the raw text syntax.

The **Wheel options** text box remains editable for advanced use and accepts one option per line. Basic lines use `Name=DMX` or `Name=start-end`, for example `Open=0-15`. Richer OFL-style metadata can be added after pipe characters. Use `kind=WheelSlot` with `slot=2` for a named slot, `kind=WheelShake` with `shake=slow-fast` for a bounded shake speed range, and `kind=WheelRotation` with `speed=slow CW-fast CW` for a bounded rotation range. For example: `Gobo 2 shake=125-140|kind=WheelShake|slot=2|shake=slow-fast`.
