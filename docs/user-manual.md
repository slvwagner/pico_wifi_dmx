...
6. Use the separate **Tile appearance** box to choose the saved toolbox tile background. Optionally upload an icon or draw one on the canvas; **No icon** clears it and **Default background** returns to the matrix's first pixel color. A live grid overlays the icon using the current Width × Height. **Contain** shows the letterboxed image area, **Cover** shows the source crop, and **Stretch** shows the full distorted image. Drawing follows the displayed transform, while the grid lines remain guides and are never stored in the icon. Tile appearance is independent from the picture's pixel colors and is reused on Controller, Chaser, and Show Run. To reuse that artwork as DMX content, click **Use icon as matrix**. The editor composites the icon over its background, resamples it with the selected Fit and Brightness, updates the pixel cells, and previews the result immediately.
