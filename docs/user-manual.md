...
```powershell
.\scripts\backup_show.ps1 -BaseUrl http://YOUR-XAMPP-HOST/dmx/ -AllowProtectedEnvironment
```
...
The file also stores the project name, project version, minimum compatible application version, export time, and setup format version. Before showing the confirmation or writing anything to the server, the controller checks both compatibility versions. Older supported setup formats are upgraded automatically to the current format. Import stops and asks you to update the controller software when either the setup format is newer than the installed software understands or the show requires a newer application release. A malformed minimum-version value is also rejected instead of importing without a reliable compatibility check.
