...
In layout edit mode, the top of Show Run has a page-level card matrix. Use **Card cols** and **Card rows** to choose how many operator cards fit in the page grid. Click **Move Cards** to arrange whole cards, for example moving **Palettes** to the top-left and **Pico Effects Playback** to the top-right. Each card shows a small **Move** handle while Move Cards is active; use that handle when a card contains interactive controls such as Live Controls faders, buttons, or dropdowns. Moving a card to an empty matrix spot places it there and leaves the other card positions unchanged. Moving a card onto another card swaps only those two cards. On touch screens, tap the card or its Move handle and then tap the destination card position.
